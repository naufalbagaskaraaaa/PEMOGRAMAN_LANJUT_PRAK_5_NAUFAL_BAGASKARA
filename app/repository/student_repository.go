...
var studentKolomUrut = map[string]string{
	"id":         "id",
	"nim":        "nim",
	"name":       "name",
	"grade":      "grade",
...
func buildStudentFilter(q model.ListQuery) (string, []any) {
	...
	if q.Search != "" {
		where += fmt.Sprintf(" AND (name ILIKE $%d OR nim ILIKE $%d)", len(args)+1, len(args)+1)
...
func (r *studentPostgresRepository) FindAll(ctx context.Context, q model.ListQuery) ([]model.Student, int, error) {
	...
	kolom, ok := studentKolomUrut[q.Sort]
	if !ok {
		kolom = "id"
	}
	...
	if err := rows.Err(); err != nil {
		return nil, 0, fmt.Errorf("membaca daftar mahasiswa: %w", err)
	}
